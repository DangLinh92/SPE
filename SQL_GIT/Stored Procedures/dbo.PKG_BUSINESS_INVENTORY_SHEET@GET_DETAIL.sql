			    ...
			    BEGIN
					SELECT sl.SPARE_PART_CODE,sp.NAME_VI,sl.CONDITION_CODE,sl.TIME_IN, 
			          (CASE sl.[EXPIRED_DATE] 
				       ...
				       END) AS [EXPIRED_DATE],CAST(GETDATE() as date) as INVENTORY_ASSETS_TIME,sl.UNIT_ID as UNIT,SL.LOCATION_CODE AS LOCATION,SUM(sl.QUANTITY) AS QUANTITY, SUM(SD.QUANTITY_REAL) as QUANTITY_REAL, SUM(SD.QUANTITY_REAL) - SUM(sl.QUANTITY) AS DIFF,sl.PO_NO
					...
					left JOIN  [dbo].[EWIP_INVENTORY_SHEET_DETAIL] SD  ON SD.DEPT_CODE = sl.DEPT_CODE AND SD.SPARE_PART_CODE = sl.SPARE_PART_CODE and SD.STOCK_CODE = sl.STOCK_CODE 
					           AND ISNULL(SD.LOCATION,'') = ISNULL(SL.LOCATION_CODE,'') AND SL.CONDITION_CODE = SD.CONDITION_CODE and  [SHEET_ID] = @A_SHEET_ID AND SL.TIME_IN = SD.TIME_IN AND SL.UNIT_ID = SD.UNIT AND ISNULL(SD.PO_NO,'') = ISNULL(SL.PO_NO,'')
							   AND (CASE WHEN SL.EXPIRED_DATE = '2199-01-01' THEN ''
								...
								GROUP BY SL.DEPT_CODE,SL.STOCK_CODE,sl.SPARE_PART_CODE,SL.LOCATION_CODE,sp.NAME_VI,sl.CONDITION_CODE,sl.TIME_IN,sl.UNIT_ID,sl.[EXPIRED_DATE],sl.PO_NO
				END
			ELSE
			    BEGIN
					SELECT [SPARE_PART_CODE],SP.NAME_VI ,CONDITION_CODE,TIME_IN, 
							(CASE [EXPIRED_DATE] WHEN '2199-01-01' THEN NULL
							                     WHEN '1900-01-01' THEN NULL
							 ELSE [EXPIRED_DATE]
							 END) AS [EXPIRED_DATE],S.[DATE_START] AS INVENTORY_ASSETS_TIME,SD.UNIT,SD.LOCATION,SD.QUANTITY,cast(SD.QUANTITY_REAL as nvarchar) as QUANTITY_REAL,SD.DIFF,SD.[PO_NO]
