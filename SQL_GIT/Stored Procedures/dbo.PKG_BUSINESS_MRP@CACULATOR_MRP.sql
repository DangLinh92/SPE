...
BEGIN TRY
		BEGIN
			...
			DECLARE @PREPARE_TIME INT = 7 -- cho duyet chi phi
							  ...
							  PRINT @A_STOCK
							  	...
							  	WHILE @@FETCH_STATUS = 0
							  		  BEGIN
											...
											SELECT @WORKING_A_DAY_PACK = ISNULL([WORKING_A_LINE_PACK]*[WORKING_LINE_NUMBER]*ISNULL([RATE],1),0) ,@WORKING_MONTH = ISNULL([WORKING_A_MONTH],0)
											...
											IF @WORKING_A_DAY_PACK IS NULL OR @WORKING_A_DAY_PACK = 0
											  BEGIN
													SELECT TOP 1 @WORKING_A_DAY_PACK = [WORKING_A_LINE_PACK]*[WORKING_LINE_NUMBER]*ISNULL([RATE],1), @WORKING_MONTH = ISNULL([WORKING_A_MONTH],0)
													FROM [dbo].[EWIP_BOM_BY_MONTH]
											...
											--SET [MIN_STOCK] = [dbo].[CONVERT_UNIT]('PACK',UNIT_CODE,SPARE_PART_CODE,@A_DEPARTMENT) * (@LEADER_TIME + 7)*@WORKING_A_DAY_PACK/2,[DATE_UPDATE] = SYSDATETIME() -- (LEAD TIME + DRAF PO = 7)/2
											--WHERE [SPARE_PART_CODE] = @SPARE_PART_CODE AND [DEPT_CODE] = 'SMT'
											...
											IF @POINT < 0
													BEGIN
														...
														SET @QTY_NEED_BUY = CEILING(@OFFSET_QUANTITY)
