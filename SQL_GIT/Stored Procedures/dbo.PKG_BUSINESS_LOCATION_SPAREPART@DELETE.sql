...
BEGIN TRY
		BEGIN
			...
			SELECT @QUANTITY = QUANTITY FROM [EWIP_SPAREPART_LOCATION] WHERE ID = @A_STT
			...
			IF @CHECK > 0 
			    BEGIN
						...
						VALUES(@A_SPARE_PART_CODE,@A_LOCATION,'DELETE',@A_CONDITION,@QUANTITY,@A_DEPART_MENT,@A_STOCK,@A_UNIT,@A_EXPIRED_DATE,@A_TIME_IN,SYSDATETIME(),@A_USER,'')
				END
			ELSE
			   IF @CHECK1 > 0
					 BEGIN
