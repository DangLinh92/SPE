...
@A_STOCK                       VARCHAR(50),
@A_DATA                       SPAREPART_LOCATION_TYPE READONLY,
@N_RETURN			int				OUTPUT,
...
BEGIN TRY
   ...
   --         SELECT 
			...
			ON (
				...
				TARGET.TIME_IN = SOURCE.TIME_IN AND
				TARGET.PO_NO = SOURCE.PO_NO
				)
			WHEN MATCHED AND (ISNULL(TARGET.[LOCATION_CODE],'') = ISNULL(SOURCE.[LOCATION_CODE],'') AND TARGET.[LOCATION_CODE] <> '') OR ((TARGET.[LOCATION_CODE] = '' OR TARGET.[LOCATION_CODE] IS NULL) AND (SOURCE.[LOCATION_CODE] = '' OR SOURCE.[LOCATION_CODE] IS NULL))
			   THEN UPDATE SET TARGET.[QUANTITY] = TARGET.[QUANTITY] + CAST(SOURCE.[QUANTITY] AS float)
			WHEN NOT MATCHED BY TARGET
			   THEN INSERT([SPARE_PART_CODE],[LOCATION_CODE],[CONDITION_CODE],[IS_WAITE],[QUANTITY],[DEPT_CODE],[STOCK_CODE],[BARCODE],[UNIT_ID],[EXPIRED_DATE],TIME_IN,PO_NO)
			        VALUES (SOURCE.[SPARE_PART_CODE],
					        ISNULL(SOURCE.[LOCATION_CODE],''),
							...
							SOURCE.[UNIT_ID], cast(SOURCE.[EXPIRED_DATE] as date),cast(SOURCE.[TIME_IN] as date),SOURCE.PO_NO);
		END
