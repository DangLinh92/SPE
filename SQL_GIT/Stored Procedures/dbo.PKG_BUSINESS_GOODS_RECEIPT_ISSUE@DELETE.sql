...
@A_DEPARTMENT_INPUT NVARCHAR(50),
@N_RETURN	int				OUTPUT,
			...
			BEGIN TRAN;
					  ...
					  BEGIN
					     if @A_INOUT = 'IN'
						   BEGIN
								...
								WHERE IVR.CODE_NO = @A_RECEIPT_ISSUE_CODE AND IVR.DEPT_CODE = @A_DEPARTMENT_INPUT
							END
						  ELSE
						    BEGIN
								INSERT INTO @A_DATA
					...
					IF @A_STATUS = 'COMPLETE'
					  BEGIN
						...
						SELECT [DEPT_CODE],[STOCK_CODE],[SPARE_PART_CODE],[UNIT],[CREATE_DATE],YEAR([CREATE_DATE]) AS [YEAR],MONTH([CREATE_DATE]) AS [MONTH],[USER_CREATE],[LOCATION],[QUANTITY_NG],[EXPRIRED_DATE],[QUANTITY],[IS_INTEGRATED],ORDER_CODE
						FROM @A_DATA

						OPEN cursorInventory

						FETCH NEXT FROM cursorInventory
						INTO @A_DEPT_CODE,@A_STOCK_CODE,@A_SPARE_PART_CODE,@A_UNIT,@A_DATE,@A_YEAR,@A_MONTH,@A_USER_CREATE,@A_LOCATION,@A_QUANTITY_NG,@A_EXPIRED_DATE,@A_QUANTITY_IN,@A_IS_INTEGRATED,@A_PO_NO
													...
													WHERE 
													 ...
													 ELSE
													 	  BEGIN
													 			...
													 			VALUES(@A_SPARE_PART_CODE,@A_TMP_L,@A_TMP_C,@A_ISWAITE,cast(@A_TMP_Q as float),@A_DEPT_CODE,@A_STOCK_CODE,'',@A_TMP_UNIT,@A_TMP_EX,@A_TMP_IT,ISNULL(@A_TMP_PO_NO,''))
													 	  END
															FETCH NEXT FROM locationCursor
															INTO @A_TMP_LOCATION
														END
