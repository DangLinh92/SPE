...
BEGIN TRY
		BEGIN
           ...
           PRINT @MONTH_PIVOT
			...
			INNER JOIN [dbo].[EWIP_PURCHASE_REQUEST_DETAIL] PRD ON SP.CODE = PRD.SPAREPART_CODE AND SP.SP_DEPT_CODE = PRD.DEPT_CODE AND ODP.[PR_CODE] = PRD.PR_CODE
			WHERE SP.SP_DEPT_CODE = '''+@A_DEPARTMENT+''' AND CAST(FORMAT(ODP.[DATE_CREATE], ''yyyyMM'') AS int) >= CAST((CAST((YEAR(ODP.[DATE_CREATE])) AS nvarchar) + ''01'') AS int) AND CAST(FORMAT(ODP.[DATE_CREATE], ''yyyyMM'') AS int) <= CAST(FORMAT(CAST('''+@A_DATE +''' AS date), ''yyyyMM'') AS int) AND SP.CODE IN (SELECT TRIM(VALUE) FROM STRING_SPLIT('''+@A_LIST_SPARE_PART+''',''$''))
			) AS TAB1
			PIVOT
			(
			...
			INNER JOIN [dbo].[EWIP_ORDER_PR] ODP ON SP.SP_DEPT_CODE = ODP.[DEPT_CODE]
			INNER JOIN [dbo].[EWIP_PURCHASE_REQUEST_DETAIL] PRD ON SP.CODE = PRD.SPAREPART_CODE AND SP.SP_DEPT_CODE = PRD.DEPT_CODE AND ODP.[PR_CODE] = PRD.PR_CODE
			WHERE SP.SP_DEPT_CODE = '''+@A_DEPARTMENT +''' AND SP.CODE IN (SELECT TRIM(VALUE) FROM STRING_SPLIT('''+@A_LIST_SPARE_PART+''',''$'')) AND ((YEAR(ODP.[DATE_CREATE]) = YEAR(CAST('''+@A_DATE+''' AS DATE))) OR (YEAR(ODP.[DATE_CREATE]) = YEAR(CAST('''+@A_DATE +''' AS DATE)) - 1))
