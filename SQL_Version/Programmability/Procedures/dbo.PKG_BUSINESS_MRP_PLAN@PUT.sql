		...
		BEGIN
		   ...
		   IF @CHECK > 0
		     BEGIN
				...
				DECLARE @QUANTITY_NEED_BUY_CS NVARCHAR(50)
				...
				WHILE @@FETCH_STATUS = 0
					 BEGIN
						 ...
						 SET @TOTAL_VN += @PRICE_VN * @QUANTITY_NEED_BUY_CS
						 SET @TOTAL_US += @PRICE_US * @QUANTITY_NEED_BUY_CS

						 IF @CHECK_EXIST > 0
						    BEGIN
								 UPDATE [dbo].[EWIP_PURCHASE_REQUEST_DETAIL]
								 SET [QUANTITY_NEED_BUY] = @QUANTITY_NEED_BUY_CS , [UNIT] = @UNIT_CS,[DATE_NEED_FINISH] = @DATE_NEED_FINISH_CS,[STATUS] = @STATUS_CS,
								     [PRICE_VN] = @PRICE_VN/[dbo].[CONVERT_UNIT](@UNIT_PRICE,@UNIT_CS,@SPAREPART_CODE_CS,@A_DEPARTMENT),
									  ...
									  [TOTAL_MONEY_VN] = (@PRICE_VN/[dbo].[CONVERT_UNIT](@UNIT_PRICE,@UNIT_CS,@SPAREPART_CODE_CS,@A_DEPARTMENT)) * @QUANTITY_NEED_BUY_CS,
									  [TOTAL_MONEY_US] = (@PRICE_US/[dbo].[CONVERT_UNIT](@UNIT_PRICE,@UNIT_CS,@SPAREPART_CODE_CS,@A_DEPARTMENT)) * @QUANTITY_NEED_BUY_CS
								 WHERE [MRP_CODE] = @MRP_CS AND [DEPT_CODE] = @A_DEPARTMENT AND [SPAREPART_CODE] = @SPAREPART_CODE_CS
							END
						 ELSE
						    BEGIN
