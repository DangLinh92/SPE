...
BEGIN TRY
		BEGIN
			...
			SELECT sl.SPARE_PART_CODE,sp.NAME_VI,sl.CONDITION_CODE,TIME_IN, 
				 ...
				 ELSE [EXPIRED_DATE]
				 END) AS [EXPIRED_DATE],CAST(GETDATE() as date) as INVENTORY_ASSETS_TIME,sl.UNIT_ID as UNIT,SL.LOCATION_CODE AS LOCATION,SUM(sl.QUANTITY) AS QUANTITY, '0' as QUANTITY_REAL, SUM(sl.QUANTITY) AS DIFF
